...
export default function PokemonGridPage() {
    ...
    const { pokemonList, loading, error, currentPage, totalPages } = useAppSelector((state) => state.pokemon)
            ...
            <div className="container mx-auto px-4 py-8">
                ...
                {!loading && filteredPokemon.length > pokemonPerPage && (
                    ...
                    >
                        ...
                        {Array.from({ length: Math.ceil(filteredPokemon.length / pokemonPerPage) }, (_, i) => i + 1).map((page) => (
                            <button
                                key={page}
                                onClick={() => handlePageChange(page)}
                                className={`px-4 py-2 rounded-lg transition-colors ${currentPage === page ? "bg-white text-red-600 font-bold" : "bg-white/10 text-white hover:bg-white/20"
                                    }`}
                            >
                                {page}
                            </button>
